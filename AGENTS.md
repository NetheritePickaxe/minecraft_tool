...
- **非必要不造轮子**：优先复用成熟方案，避免重复实现已有功能
- **借鉴知名开源项目**：遇到通用需求（如 i18n、状态管理、路由、文件监听、加解密等）先调研生态中成熟的库或知名项目的实现方式，再决定是否自行实现
- 引入第三方依赖前应简要评估：维护活跃度、包体积、与 Tauri v2 / Vite 栈的兼容性、许可证
- 仅在以下情况考虑自行实现：需求高度定制、现有方案过重或已停止维护、仅需极简功能且实现成本低于集成成本
...
- 模块的语言文件**不放在模块目录内**，统一放 `src/lang/locales/`（见第 6 节），按命名空间区分
